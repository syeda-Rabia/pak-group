...
import RecordingFileForLead from "../../../components/RecordingFileForLead";
...
// console.log("----------------------props----------------",props?.location?.data?.data);
  ...
  const getAllLeadsData = async () => {
    ...
    let resp = await GET(ApiUrls.GET_EMPLOYE_RECORDING_ON_LEAD+ "/" +leadID+"/"+"empRecording"+"/"+ActionId);
    ...
    <Container fluid>
      ...
      <Row className="shadow p-3 mb-3 bg-white rounded mt-4 ">
        ...
        <div className="col-md-6 col-sm-12">
          {allactions.length > 0 ? (
            <RecordingFileForLead data={allactions[0]} />
          ) : null}
        </div>
